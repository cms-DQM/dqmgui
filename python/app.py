                ...
                "items": [
                    {
                        "run": str(sample.run)
                        if sample.lumi == 0
                        else "%s:%s" % (sample.run, sample.lumi),
...
async def archive_legacy(request, notOlderThan):
    ...
    data = await service.get_archive(
        run, dataset, path, search, lumi, notOlderThan=notOlderThan
    )
    if not data:
    ...
    result["contents"].extend(
        {"subdir": name, "me_count": me_count} for name, me_count in data.dirs
    )
    ...
    for (name, path, layout, qteststatuses) in data.objs:
        ...
        if layout != None:
            obj["draw"] = {
                k: v for k, v in zip(layout.draw._fields, layout.draw) if v is not None
            }
...
async def layouts_v1(request):
    ...
    result = {
        "data": [{"source": x.source, "destination": x.destination} for x in layouts]
    }
...
async def render_legacy(request, notOlderThan):
    ...
    data, error = await service.get_rendered_image(
        [me_description], options, notOlderThan=notOlderThan
    )
    ...
    return web.Response(
        body=data, content_type="image/png", status=200 if error == 0 else 500
    )
...
async def render_overlay_legacy(request, notOlderThan):
    ...
    data, error = await service.get_rendered_image(
        me_descriptions, options, notOlderThan=notOlderThan
    )
...
async def jsroot_legacy(request, notOlderThan):
    ...
    data, error = await service.get_rendered_json(
        [me_description], options, notOlderThan=notOlderThan
    )
...
async def exception_handler_middleware(request, handler):
    try:
        ...
        return web.json_response(
            {
                "error": "The server is under a heavy load at the moment, please try again later."
            },
            status=429,
        )
...
def config_and_start_webserver(port):
    ...
    app.add_routes(
        [
            web.get("/data/json/samples", samples_legacy),
    ...
    app.add_routes(
        [
            web.get("/api/v1/samples", samples_v1),
    ...
    app.add_routes(
        [
            web.get("/", index),
            web.static("/", get_absolute_path("../frontend/"), show_index=True),
        ]
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=int,
        default=2,
        help="Number of renderer processes.",
    )
    parser.add_argument(
        "--in-memory",
        dest="in_memory",
        default=False,
        action="store_true",
        help="If set uses an in memory database.",
    )
    parser.add_argument(
        "--stderr",
        default=False,
        action="store_true",
        help="If set log to stdout instead of log files.",
    )
    parser.add_argument(
        "-d",
        dest="workdir",
    ...
    parser.add_argument(
        ...
        help="Host wehere DQM GUI is running",
    )
    ...
    asyncio.get_event_loop().run_until_complete(
        initialize_services(workdir, args.in_memory, args.files, args.renderers)
    )
