...
def start(directory, cmsswSymlink, threshold, lower_threshold, life_time, alarm_email, alarm_email_reload_time) :
    print('Starting to watch directory: %s' % directory)
    email_time        = -alarm_email_reload_time * 60 - 1
            ...
            if allPBFiles :
                ...
                if used_space > threshold * total_space:
                    ...
                    if (used_space - freed_space) > lower_threshold * total_space:
                        print('dqmgui-cleanup.py: can"t reach the target fraction of disk free space, no more PB files to delete! (', (used_space - freed_space), "Mb >", lower_threshold * total_space, "Mb)")
                        if alarm_email and (local_time - email_time) > alarm_email_reload_time * 60:
                            email_time = local_time
                            from socket import gethostname
                            from subprocess import Popen, PIPE
                            def get_percent(val):
                                return str(int(val * 100/ max(total_space, 1))) + "%"

                            header  = "To: %s\n" % alarm_email
                            header += "Subject: Message from the dqmgui-cleanup.py on %s at P5\n\n" % gethostname()
                            msg  = "dqmgui-cleanup.py: can't reach the safe fraction of disk availalbe space, no more PB files to delete!"
                            msg += "\n  Total space: " + str(total_space) + "Mb 100%"
                            msg += "\n  Used space: " + str(used_space - freed_space) + "Mb " + get_percent(used_space - freed_space)
                            msg += "\n  Available space: " + str(total_space - used_space + freed_space) + "Mb " + get_percent(total_space - used_space + freed_space)
                            msg += "\n  Required space: " + str(total_space - lower_threshold * total_space) + "Mb " + get_percent(total_space - lower_threshold * total_space)
                            msg += "\nThe .pb directory: " + directory + "\n"
                            msg += "-------------------\nBest regards, dqmgui-cleanup.py\n\n"
                            print("dqmgui-cleanup.py: Sending email to %s with following message:\n\"%s\"" % (alarm_email, msg))
                            ...
                            if returncode != 0:
                                print("dqmgui-cleanup.py: ERROR: Sendmail exit with status %s" % returncode)
                        else:
                            print("dqmgui-cleanup.py: Not sending email")
...
if __name__ == '__main__':
    ...
    parser.add_argument('-l', '--lower_threshold', default=-1.0, type=float, help='Target fraction of used space in disk; will try to clean up up to this fraction.')
    parser.add_argument('--life_time', default=10, type=int, help='Minimum life time values of .pb files before deletion, in Minutes')
    parser.add_argument('--alarm_email', default="cms-dqm-oncall@cern.ch", type=str, help="Emails to notify, multiple addresses are accepted, separated by ','")
    parser.add_argument('--alarm_email_reload_time', default=30, type=int, help='Minimum delay between two alarm emails, in Minutes')
    args = parser.parse_args()

    if args.lower_threshold < 0 : args.lower_threshold = args.threshold
    start(args.directory, args.cmsswSymlink, args.threshold, args.lower_threshold, args.life_time, args.alarm_email, args.alarm_email_reload_time)
...
[1] For the debugging you can populate random file like : head -c 100M </dev/urandom > run341343_DQMLive_concat_b25f7ad9934658ca42f93f07407b372f.pb
    Moreover, you can use dqmgui-cleanup.py script to emulate PB files creation.
"""
