...
# ============================================================================================================== #
# =============================== Synchronous implementation of Protobuf parser ================================ #
# ============================================================================================================== #
...
def start(file, output):
    ...
    histo_messages = protobuf_parser.deserialize_file(file, uncompress_only_scalars=True)

    with open(output, 'wb') as buffer:
        for message in histo_messages:
            type_byte = message.flags & 255
            is_scalar = type_byte in (1, 2, 3)
            if is_scalar:
                streamed_histo = message.streamed_histo
                # size = message.size
                size = len(message.streamed_histo)
            else:
                streamed_histo = b''
                size = 0

            buffer.write(message.message_size.to_bytes(4, byteorder='little'))
            buffer.write(message.flags.to_bytes(4, byteorder='little'))
            buffer.write(message.offset.to_bytes(4, byteorder='little'))
            buffer.write(len(message.full_pathname).to_bytes(4, byteorder='little'))
            buffer.write(size.to_bytes(4, byteorder='little'))
            buffer.write(message.full_pathname)
            buffer.write(streamed_histo)


if __name__ == '__main__':
    parser = argparse.ArgumentParser(description=
        '''This utility takes a merged PB file containing PB output from multiple DQM clients, and pre-imports it.
        The notion of pre-importing means that a compact file is produced containing only the metadata of the histograms.
        The metadata is exactly the data that the regular importing extracts from a full PB file. 
        Having a compact file means that the IO footprint of importing on demand is much smaller, and hence it is much quicker.
        It's adviced to perform this procedure immediately after the merged file is produced, as its contents are still cached by multiple layers of the system. This reduces the impact of otherwise expensive IO operations.
        
        This utility outputs a new line separated binary file that always contains 5 consecutive lines for each entry in the original PB file.
        Each entry follows this (strict) order of fields for ease of parsing: full_pathname, size, streamed_histo, flags, offset, message_size.
        streamed_histo field is included only if the histogram is of scalar type.''')
    parser.add_argument('-f', '--file', type=str, required=True, help='Input file to pre-import.')
    parser.add_argument('-o', '--output', type=str, help='Output file path. The default is the same as the input with .meta extension.')
    args = parser.parse_args()

    start(args.file, args.output)
